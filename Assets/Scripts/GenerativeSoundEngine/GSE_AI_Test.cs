...
namespace GenerativeSoundEngine
{
    public class GSE_AI_Test
    {
        ...
        public void Timer(DateTime startDate, DateTime nowDate)
        {
            ...
            if ((lastTimePassed.TotalSeconds < 3) && (3 <= timePassed.TotalSeconds))
            {
                stateEngine = true;
            } else if ((lastTimePassed.TotalSeconds < 7) && (7 <= timePassed.TotalSeconds))
            {
                stateBlinker = 0.0f;
            } else if ((lastTimePassed.TotalMilliseconds < 11600) && (11000 <= timePassed.TotalMilliseconds))
            {
                stateSpeed = 5.0f;
                stateThrottle = 0.33f;
                stateSteering = (float)(timePassed.TotalMilliseconds - 11000) * -0.05f;
            } else if ((lastTimePassed.TotalMilliseconds < 13300) && (11600 <= timePassed.TotalMilliseconds))
            {
                stateSteering = -30.0f;
            } else if ((lastTimePassed.TotalMilliseconds < 14300) && (13300 <= timePassed.TotalMilliseconds))
            {
                stateSteering = (float)(timePassed.TotalMilliseconds - 14300) * 0.03f;
            } else if ((lastTimePassed.TotalMilliseconds < 14400) && (14300 <= timePassed.TotalMilliseconds))
            {
                stateSteering = 0.0f;
            } else if ((lastTimePassed.TotalMilliseconds < 15389) && (14400 <= timePassed.TotalMilliseconds))
            {
                stateSteering = (float)(timePassed.TotalMilliseconds - 14400) * 0.03f;
            } else if ((lastTimePassed.TotalMilliseconds < 16378) && (15389 <= timePassed.TotalMilliseconds))
            {
                stateSteering = (float)(timePassed.TotalMilliseconds - 16378) * -0.03f;
            } else if ((lastTimePassed.TotalSeconds < 17) && (16378 <= timePassed.TotalMilliseconds))
            {
                stateBlinker = 0.5f;
                stateSteering = 0.0f;
                stateSpeed = 30.0f;
                stateThrottle = 0.66f;
            } else if ((lastTimePassed.TotalSeconds < 30) && (30 <= timePassed.TotalSeconds))
            {
                stateSpeed = 0.0f;
                stateThrottle = 0.0f;
                stateBreaking = 0.8f;
